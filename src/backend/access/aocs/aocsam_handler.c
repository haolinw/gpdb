                                  ...
                                  TableScanDesc scan)
{
	...
	else if (numblocks != InvalidBlockNumber)
	{
		...
		for (int colIdx = 0; colIdx < aocoscan->columnScanInfo.num_proj_atts; colIdx++)
		{
			...
			{
				/*
				 * Since we found a block directory entry at/prior to the
				 * start_blockno, we can attempt to jump ahead.
				 */
			...
			else
			{
				 ...
				 * segfile).
				 *
				 * Do nothing and break in this case. The scan will start anyway
				 * from the first row present in the segfile. This will ensure
				 * that we don't skip the other possibly extant blocks in the
				 * range.
				 */
				break;
			}
