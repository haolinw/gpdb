			...
			bool is_new_table)
{
	...
	if (stmt->unique && RelationIsAppendOptimized(rel))
	{
		...
		if (OidIsValid(blkdirrelid) && !ValidateAORelationVersion(rel, AORelationVersion_GP7))
		{
			/*
			...
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						...
						errhint("re-create table and reload the data before creating the unique index")));
		}
	}
