...
typedef struct AlteredTableInfo
{
	...
	bool		aocs_write_new_columns_only; /* T for ADD COLUMN on AOCS relation */
...
ExecuteTruncate(TruncateStmt *stmt)
{
	...
	foreach(cell, rels)
	{
		...
		if (RelationIsAppendOptimized(rel) && IS_QUERY_DISPATCHER())
		{
			...
			if (!IsAORelHashEntryInUse(RelationGetRelid(rel)))
...
ATRewriteTables(List **wqueue, LOCKMODE lockmode)
{
	...
	foreach(ltab, *wqueue)
	{
		...
		if (tab->rewrite && tab->aocs_write_new_columns_only)
		{
		...
		if (aocs_write_new_columns_only)
		{
			...
			foreach (lc, all_inheritors)
			{
				...
				if (rel->rd_rel->relstorage == RELSTORAGE_AOCOLS)
					childtab->aocs_write_new_columns_only = true;
				heap_close(rel, NoLock);
			}
		}
	}
