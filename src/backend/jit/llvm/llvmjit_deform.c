                    ...
                    const TupleTableSlotOps *ops, int natts)
{
    ...
    LLVMBasicBlockRef *attgetdataptrvarblocks;
    ...
    /* Memtuple does not store attributes in logical order, so we need to deform all attributes. */
    natts = desc->natts;
    ...
    attgetdataptrvarblocks = palloc(sizeof(LLVMBasicBlockRef) * natts);
    ...
    for (attnum = 0; attnum < natts; attnum++)
    {
        ...
        attgetdataptrvarblocks[attnum] =
            l_bb_append_v(v_deform_fn, "block.att.%d.attgetdataptrvar", attnum);
    ...
    for (attnum = 0; attnum < natts; attnum++)
    {
        ...
        v_bindingflag =
            l_load_struct_gep(b, v_attrbindp, FIELDNO_MEMTUPLEATTRBINDING_FLAG, "binding_flag");
        v_nullbyteno =
            ...
            LLVMBuildCondBr(b,
                            LLVMBuildOr(b,
                                        LLVMBuildICmp(b,
                                                      LLVMIntEQ,
                                                      v_bindingflag,
                                                      l_int32_const(MTB_ByVal_Native),
                                                      ""),
                                        LLVMBuildICmp(b,
                                                      LLVMIntEQ,
                                                      v_bindingflag,
                                                      l_int32_const(MTB_ByVal_Ptr),
                                                      ""),
                                        ""),
                            attfetchblocks[attnum],
                            attgetdataptrvarblocks[attnum]);
        ...
        LLVMPositionBuilderAtEnd(b, attgetdataptrvarblocks[attnum]);
        LLVMBuildCondBr(b,
